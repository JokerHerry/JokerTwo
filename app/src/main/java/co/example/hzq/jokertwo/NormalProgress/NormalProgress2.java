...
public class NormalProgress2 {
    private static final String TAG = "NormalProgress2";
    public static Handler DetailActivityHandler;
    private static BaseActivity context;

    public NormalProgress2(Handler DetailActivityHandler,BaseActivity context){
        this.DetailActivityHandler = DetailActivityHandler;
    ...
    public static void deteleface(){
        ...
        faceApi.detectFace("http://139.199.202.227:8000/static/img/photo.jpg", new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                List<Map<String, Object>> test_face_token = JsonUtil.find_test_face_token(s);

                if (test_face_token==null){
                ...
                Log.e(TAG, "onResponse: "+"发现"+test_face_token.size()+"张人脸" );


                for (int i = 0; i < test_face_token.size(); i++) {
                    Message message =new Message();
                    message.what = 123;

                    Bundle bundle = new Bundle();
                    Map<String, Object> stringObjectMap = test_face_token.get(i);
                    bundle.putIntArray("face_position",
                            new int[]{
                                    (int) stringObjectMap.get("left"),
                                    (int) stringObjectMap.get("top"),
                                    (int) stringObjectMap.get("width"),
                                    (int) stringObjectMap.get("height")
                            });
